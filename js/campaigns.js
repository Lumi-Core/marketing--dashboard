...
const Campaigns = {
    ...
    bindEvents() {
        ...
        on(pg, 'click', '.btn-import-campaigns', () => { this._importTarget = 'campaigns'; openModal('import-modal'); });
        ...
        // Modal save
        const saveBtn = $('#save-campaign-btn');
        if (saveBtn) saveBtn.addEventListener('click', () => this.saveCampaign());

        // Target modal save
        const saveTargetBtn = $('#save-target-btn');
    ...
    async openTargetModal(id) {
        ...
        try {
            ...
            if (form && targets) populateForm(form, targets);
            const title = $('#target-modal-title');
            if (title) title.textContent = `Targets — Campaign #${id}`;
    ...
    async saveTargets() {
        const form = $('#target-form');
        if (!form || !this._targetCampaignId) return;
        const data = getFormData(form);
        try {
