...
const Analytics = {
    ...
    renderKPIs(container, data) {
        if (!data || !data.campaign_metrics) {
            container.innerHTML = '<p class="text-muted">No KPI data available yet. Run some campaigns to see metrics.</p>';
            return;
        }
        ...
        html += `<div class="kpi-card kpi-primary">
            <div class="kpi-header">
                ...
                <div class="kpi-trend ${(cm.growth_rate || 0) >= 0 ? 'positive' : 'negative'}">
                    <i class="fas fa-arrow-${(cm.growth_rate || 0) >= 0 ? 'up' : 'down'}"></i>
                    ${Math.abs(cm.growth_rate || 0)}%
                </div>
            </div>
            <div class="kpi-value">${formatNumber(cm.total_campaigns || 0)}</div>
            <div class="kpi-label">Total Campaigns</div>
            <div class="kpi-details">
                <span class="kpi-detail-item"><i class="fas fa-check-circle"></i> ${cm.completed || 0} completed</span>
                <span class="kpi-detail-item"><i class="fas fa-times-circle"></i> ${cm.failed || 0} failed</span>
            </div>
            <div class="kpi-footer">Success Rate: ${formatPercent((cm.success_rate || 0) * 100)}</div>
        </div>`;

        html += `<div class="kpi-card kpi-success">
            <div class="kpi-header">
                <div class="kpi-icon"><i class="fas fa-paper-plane"></i></div>
                <div class="kpi-trend ${(vm.growth_rate || 0) >= 0 ? 'positive' : 'negative'}">
                    <i class="fas fa-arrow-${(vm.growth_rate || 0) >= 0 ? 'up' : 'down'}"></i>
                    ${Math.abs(vm.growth_rate || 0)}%
                </div>
            </div>
            <div class="kpi-value">${formatNumber(vm.total_sent || 0)}</div>
            <div class="kpi-label">Messages Sent</div>
            <div class="kpi-details">
                <span class="kpi-detail-item">Avg: ${formatNumber(vm.avg_per_campaign || 0)}/campaign</span>
            </div>
            <div class="kpi-footer">Last ${data.period_days || 30} days</div>
        </div>`;

        html += `<div class="kpi-card kpi-info">
            <div class="kpi-header">
                <div class="kpi-icon"><i class="fas fa-users"></i></div>
                <div class="kpi-badge">${(em.audience_segments || 0)} segments</div>
            </div>
            <div class="kpi-value">${formatNumber(em.active_clients || 0)}</div>
            <div class="kpi-label">Active Clients</div>
            <div class="kpi-details">
                <span class="kpi-detail-item">Activity Rate: ${formatPercent((em.active_rate || 0) * 100)}</span>
            </div>
        ...
        html += `<div class="kpi-card kpi-warning">
            <div class="kpi-header">
                ...
                <div class="kpi-badge">${formatPercent(am.adoption_rate || 0)}% adopted</div>
            </div>
            <div class="kpi-value">${formatNumber(am.total_recommendations || 0)}</div>
            <div class="kpi-label">AI Recommendations</div>
            <div class="kpi-details">
                <span class="kpi-detail-item"><i class="fas fa-check"></i> ${am.applied || 0} applied</span>
            </div>
            <div class="kpi-footer">Avg Confidence: ${formatPercent(am.avg_confidence || 0)}</div>
        </div>`;
    ...
    renderStrategicInsights(container, data) {
        if (!data || !data.top_recommendations) {
            container.innerHTML = '<p class="text-muted">No strategic insights available yet. Run campaigns to generate insights.</p>';
            return;
        }
        ...
        if (data.top_recommendations.length) {
            ...
            data.top_recommendations.slice(0, 5).forEach((rec, i) => {
                ...
                html += `<div class="priority-rec-card priority-${priority}">
                    ...
                    <div class="priority-confidence"><i class="fas fa-brain"></i> ${formatPercent(rec.confidence)} confidence</div>
        ...
        if (data.best_performing_patterns && data.best_performing_patterns.length) {
            ...
            data.best_performing_patterns.forEach(pattern => {
                html += `<div class="pattern-card">
                    ...
                    <div class="pattern-stats">${pattern.campaign_count} campaigns • ${formatNumber(pattern.total_sent)} messages sent</div>
                </div>`;
            });
            html += '</div></div>';
        }
    ...
    renderEngagementBreakdown(container, data) {
        if (!data || !data.summary) {
            container.innerHTML = '<p class="text-muted">No engagement data available yet.</p>';
            return;
        }
        ...
        html += `<div class="engagement-stat"><div class="eng-stat-value">${formatNumber(s.total_successful)}</div><div class="eng-stat-label">Successful Sends</div></div>`;
        html += `<div class="engagement-stat"><div class="eng-stat-value">${formatNumber(s.total_failed)}</div><div class="eng-stat-label">Failed Sends</div></div>`;
        html += `<div class="engagement-stat highlight"><div class="eng-stat-value">${formatPercent(s.engagement_rate)}</div><div class="eng-stat-label">Engagement Rate</div></div>`;
        html += `<div class="engagement-stat"><div class="eng-stat-value">${s.unique_campaigns}</div><div class="eng-stat-label">Unique Campaigns</div></div>`;
        html += '</div></div>';
